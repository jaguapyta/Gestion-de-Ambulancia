            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {pendientes.map(p => (
                <div key={p.id} style={{ background: 'white', border: '0.5px solid #fde68a', borderRadius: '8px', padding: '10px 12px', display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>
                  ...
                  {!soloLectura && <button onClick={() => aprobar(p.id)} style={{ padding: '5px 14px', borderRadius: '7px', border: 'none', background: '#15803d', color: 'white', cursor: 'pointer', fontSize: '12px', fontWeight: 500 }}>Aprobar</button>}
                  {!soloLectura && <button onClick={() => rechazar(p.id)} style={{ padding: '5px 14px', borderRadius: '7px', border: '0.5px solid #fecaca', background: 'white', color: '#dc2626', cursor: 'pointer', fontSize: '12px' }}>Rechazar</button>}
                </div>
              ))}
            </div>
          </div>
                  ...
                  {(m.sinonimos ?? []).map((s: any) => {
                    ...
                    return (
                      <span key={s.id} style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', background: s.activo ? o.bg : '#f9fafb', color: s.activo ? o.tx : '#c0c4cc', border: '0.5px solid #f0f0f0', padding: '3px 8px', borderRadius: '20px', fontSize: '12px', textDecoration: s.activo ? 'none' : 'line-through' }}>
                        ...
                        {!soloLectura && <button onClick={() => toggle(s.id)} title={s.activo ? 'Desactivar' : 'Activar'} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'inherit', fontSize: '11px', padding: 0 }}>{s.activo ? '⊘' : '＋'}</button>}
                        {!soloLectura && <button onClick={() => borrar(s.id)} title="Borrar" style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#dc2626', fontSize: '12px', padding: 0 }}>×</button>}
                      </span>
                    );
                  })}
