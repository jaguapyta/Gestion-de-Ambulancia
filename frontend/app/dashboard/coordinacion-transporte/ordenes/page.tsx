...
export default function OrdenesPage() {
  ...
  const cargarGuardiaActiva = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setError('No hay guardia activa en este momento.');
...
<head>
  ...
  <style>
    ...
    .titulo { font-size: 16px; font-weight: bold; margin: 10px 0 6px; }
    .fila { display: flex; gap: 20px; margin-bottom: 6px; align-items: flex-end; }
    .campo { flex: 1; }
    .campo label { font-weight: bold; }
    .campo span { border-bottom: 1px solid #000; display: inline-block; min-width: 80px; min-height: 14px; }
    .seccion { margin: 10px 0; }
    .trabajos { border: 1px solid #000; min-height: 60px; padding: 6px; margin-top: 4px; }
    .firmas { display: flex; justify-content: space-between; margin-top: 40px; text-align: center; }
    .firma { width: 30%; }
    .firma-linea { border-top: 1px solid #000; padding-top: 4px; margin-top: 30px; }
    .pie { text-align: center; margin-top: 20px; font-size: 10px; border-top: 1px solid #000; padding-top: 6px; }
...
<body>
  <div class="header">
    <h3>MINISTERIO DE SALUD PÚBLICA Y BIENESTAR SOCIAL</h3>
    <h3>SERVICIO DE EMERGENCIAS MÉDICAS EXTRAHOSPITALARIAS (S.E.M.E.)</h3>
    <p><strong>DEPARTAMENTO DE TRANSPORTE</strong></p>
  </div>

  <div style="display:flex; align-items:center; gap:20px; margin-bottom:8px;">
    <div class="titulo">ORDEN DE TRABAJO N°</div>
    <div style="font-size:18px; font-weight:bold;">${orden.nro_orden}</div>
  </div>
  ...
  <div class="fila">
    ...
    <div><label>Area Asignada: </label><span>${orden.area_asignada}</span></div>
  </div>
  <div class="fila">
  ...
  <div class="fila">
    <div><label>Km. De Salida: </label><span>${orden.km_salida?.toLocaleString() ?? ''}</span></div>
  </div>
  <div class="fila">
    <div><label>Km. De Llegada: </label><span>${orden.km_llegada?.toLocaleString() ?? ''}</span></div>
  </div>
  <div class="fila">
    <div><label>Total recorrido: </label><span>${orden.km_llegada && orden.km_salida ? (orden.km_llegada - orden.km_salida).toLocaleString() : ''}</span></div>
  </div>
  <div class="fila">
  ...
  <div class="seccion">
    ...
    <div class="trabajos">${orden.trabajos ?? ''}</div>
  </div>

  <div class="firmas">
    <div class="firma">
      <div>${new Date(orden.fecha_inicio).toLocaleDateString('es-PY')}</div>
      <div class="firma-linea"><strong>FECHA</strong></div>
    </div>
    <div class="firma">
      <div>${getNombreConductor(orden).toUpperCase()}</div>
      <div class="firma-linea"><strong>CONDUCTOR</strong></div>
    </div>
    <div class="firma">
      <div>&nbsp;</div>
      <div class="firma-linea"><strong>FIRMA AUTORIZADA</strong></div>
    </div>
  </div>

  <div class="pie">
    <p>ORIGINAL: Conductor del Móvil &nbsp;&nbsp;&nbsp; Copia: Transporte</p>
    <p>Avda. Fdo. De la Mora E/ Dr. Venza &nbsp;&nbsp;&nbsp; Teléfono: 021-562-303 &nbsp;&nbsp;&nbsp; Urgencias y Emergencias: 141</p>
...
<head>
  <meta charset="UTF-8">
  <title>Anexo III - ${orden.nro_orden}</title>
  <style>
    body { font-family: Arial, sans-serif; font-size: 11px; margin: 20px; color: #000; }
    h2 { text-align: center; font-size: 14px; margin: 4px 0; }
    h3 { text-align: center; font-size: 12px; margin: 4px 0; }
    .fila { display: flex; gap: 16px; margin-bottom: 6px; align-items: flex-end; }
    .campo label { font-weight: bold; }
    .campo span { border-bottom: 1px solid #000; display: inline-block; min-width: 80px; min-height: 14px; }
    ...
    .firma-linea { border-top: 1px solid #000; padding-top: 4px; margin-top: 30px; }
    .combustible { display: flex; gap: 20px; margin: 6px 0; }
    .check-box { border: 1px solid #000; display: inline-block; width: 14px; height: 14px; text-align: center; line-height: 14px; font-size: 10px; }
    @media print { body { margin: 10px; } }
  </style>
</head>
<body>
  <h2>ANEXO : III</h2>
  <h3>PARTE DIARIO DE USO DEL VEHICULO OFICIAL</h3>

  <div class="fila" style="margin-top:10px;">
    <div class="campo"><label>FECHA: </label><span>${new Date(orden.fecha_inicio).toLocaleDateString('es-PY')}</span></div>
    <div class="campo"><label>Orden de Trabajo N°: </label><span>${orden.nro_orden}</span></div>
  </div>

  <div class="fila">
    <div class="campo"><label>TIPO DE VEHICULO: </label><span>${orden.movil.tipo}</span></div>
    <div class="campo"><label>CHAPA N°: </label><span>${orden.movil.placa ?? '—'}</span></div>
    <div>
      <label>TIPO DE COMBUSTIBLE</label>
      <div class="combustible">
        <div><span class="check-box">${orden.movil.tipo === 'AMBULANCIA' ? '' : ''}</span> NAFTA</div>
        <div><span class="check-box">X</span> GAS OIL</div>
      </div>
    </div>
  </div>

  <div class="fila">
    <div class="campo"><label>Código de Móvil: </label><span>${orden.movil.cod_movil}</span></div>
    <div class="campo"><label>R.A.S.P. N°: </label><span>${orden.movil.rasp ?? 'EN TRAMITE'}</span></div>
  </div>

  <div class="fila">
    <div class="campo"><label>MARCA: </label><span>${orden.movil.marca ?? '—'}</span></div>
    <div class="campo"><label>N° DE ORDEN ASIGNADO: </label><span>${orden.movil.nro_orden ?? 'EN TRAMITE'}</span></div>
  </div>

  <div class="fila">
    <div class="campo"><label>MODELO: </label><span>${orden.movil.modelo ?? '—'}</span></div>
    <div class="campo"><label>CONSUMO X 100 KM.: </label><span>${orden.movil.consumo_l100km ?? '—'}</span> Lts.</div>
  </div>
  ...
  <table>
    ...
    <tbody>
      <tr>
        <td>${new Date(orden.fecha_inicio).toLocaleDateString('es-PY')}</td>
        <td>${new Date(orden.fecha_fin).toLocaleDateString('es-PY')}</td>
        <td></td>
        <td>${orden.km_salida?.toLocaleString() ?? ''}</td>
        <td>${orden.km_llegada?.toLocaleString() ?? ''}</td>
        <td>${orden.km_llegada && orden.km_salida ? (orden.km_llegada - orden.km_salida).toLocaleString() : ''}</td>
        <td></td>
        <td></td>
      </tr>
  ...
  <div class="firmas">
    <div class="firma">
      ...
      <div class="firma-linea">
        <div>Conductor: <strong>${getNombreConductor(orden).toUpperCase()}</strong></div>
        <div>C.I. N°: ${orden.conductor.persona.nro_documento}</div>
      </div>
    </div>
    <div class="firma">
      <div>&nbsp;</div>
            ...
            {cargando ? (
              ...
              <tr key={o.id} style={{ borderBottom: '0.5px solid #f3f4f6' }}>
                ...
                <td style={{ padding: '12px 16px' }}>
                  <span style={{ background: o.tipo === 'ORDINARIO' ? '#eff6ff' : '#fff7ed', color: o.tipo === 'ORDINARIO' ? '#1d4ed8' : '#c2410c', padding: '3px 10px', borderRadius: '20px', fontSize: '11px', fontWeight: '500' }}>
                    {o.tipo}
                  </span>
                </td>
                ...
                <td style={{ padding: '12px 16px' }}>
                  <span style={{ background: o.estado === 'ACTIVA' ? '#f0fdf4' : '#f9fafb', color: o.estado === 'ACTIVA' ? '#15803d' : '#6b7280', padding: '3px 10px', borderRadius: '20px', fontSize: '11px', fontWeight: '500' }}>
                    {o.estado}
                  </span>
                </td>
                <td style={{ padding: '12px 16px' }}>
                  <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap' as const }}>
                    <button onClick={() => imprimirOrdenTrabajo(o)} style={{ background: 'transparent', border: '0.5px solid #e5e7eb', padding: '4px 8px', borderRadius: '6px', cursor: 'pointer', fontSize: '11px', color: '#374151' }}>
                      🖨️ Orden
                    </button>
                    <button onClick={() => imprimirAnexoIII(o)} style={{ background: 'transparent', border: '0.5px solid #e5e7eb', padding: '4px 8px', borderRadius: '6px', cursor: 'pointer', fontSize: '11px', color: '#374151' }}>
                      🖨️ Anexo III
                    </button>
                    {o.estado === 'ACTIVA' && (
                      <button onClick={() => { setOrdenSeleccionada(o); setModalCerrarAbierto(true); }} style={{ background: 'transparent', border: '0.5px solid #fecaca', padding: '4px 8px', borderRadius: '6px', cursor: 'pointer', fontSize: '11px', color: '#dc2626' }}>
                        Cerrar
                      </button>
                    )}
                  </div>
          ...
          <div style={{ background: 'white', borderRadius: '12px', padding: '28px', width: '600px', maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 8px 32px rgba(0,0,0,0.15)' }}>
            ...
            {!form.movil_id && (
              <div style={{ display: 'flex', gap: '10px', marginBottom: '20px' }}>
                <button onClick={cargarGuardiaActiva} disabled={cargandoGuardia}
                  style={{ flex: 1, padding: '12px', borderRadius: '8px', border: '0.5px solid #e5e7eb', background: '#f0f4f8', cursor: 'pointer', fontSize: '13px', fontWeight: '500', color: '#0a2540' }}>
                  {cargandoGuardia ? 'Cargando...' : '📋 Estirar desde guardia activa'}
                </button>
                <button onClick={() => setModoGuardia(false)}
                  style={{ flex: 1, padding: '12px', borderRadius: '8px', border: '0.5px solid #e5e7eb', background: '#f0f4f8', cursor: 'pointer', fontSize: '13px', fontWeight: '500', color: '#0a2540' }}>
            ...
            {/* Lista de móviles de la guardia */}
                ...
                {guardiaData.moviles.length === 0 ? (
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    {guardiaData.moviles.map((m: MovilGuardia, i: number) => (
                      <div key={i} onClick={() => handleSeleccionarMovilGuardia(m)}
                        style={{ background: '#f8f9fb', border: '0.5px solid #e5e7eb', borderRadius: '8px', padding: '12px 16px', cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <div>
                          <div style={{ fontSize: '14px', fontWeight: '600', color: '#0a2540' }}>🚑 {m.movil.cod_movil}</div>
                          <div style={{ fontSize: '12px', color: '#6b7280', marginTop: '2px' }}>
                            👤 {m.conductor.persona.primer_nombre} {m.conductor.persona.primer_apellido}
                          </div>
                        </div>
            ...
            {/* Formulario manual o con datos estirados */}
            {!modoGuardia && (
              <>
                {form.movil_id && (
