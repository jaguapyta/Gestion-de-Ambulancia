...
// Vínculos desde Excel: "1D+3D;2N+5D"  (; separa vínculos · + une los 2 turnos)
...
const getSupervisores = async (req, res) => {
  try {
    const supers = await prisma.supervisor_habilitado.findMany({
      include: {
        usuario: {
          include: {
...
const crearSupervisor = async (req, res) => {
  ...
  try {
    ...
    if (usuarioExiste) {
      const yaHab = await prisma.supervisor_habilitado.findFirst({ where: { usuario_id: usuarioExiste.id } });
      if (yaHab) return res.status(400).json({ error: 'Esta persona ya está registrada como supervisor' });

      await prisma.supervisor_habilitado.create({
        data: {
      ...
      return res.status(201).json({ mensaje: 'Usuario habilitado como supervisor correctamente' });
    }

    const rol = await prisma.rol.findFirst({ where: { nombre: 'SUPERVISOR_GUARDIA' } });
    if (!rol) return res.status(400).json({ error: 'Rol SUPERVISOR_GUARDIA no encontrado' });
...
const crearSupervisoresMasivo = async (req, res) => {
  const { supervisores } = req.body;
  if (!Array.isArray(supervisores) || supervisores.length === 0) {
    return res.status(400).json({ error: 'No se enviaron supervisores' });
  }
  ...
  for (const s of supervisores) {
    try {
      if (!s.nro_documento || !s.primer_nombre || !s.primer_apellido) {
        resultados.errores.push({ documento: s.nro_documento ?? '?', motivo: 'Datos incompletos' });
        continue;
      }

      const { filas, error: errorVinc } = normalizarVinculosArm(parseVinculos(s.vinculos));
      if (errorVinc) {
        resultados.errores.push({ documento: s.nro_documento, motivo: errorVinc });
        continue;
      }

      let personaId = null;
      const existe = await prisma.persona.findFirst({ where: { nro_documento: String(s.nro_documento) } });
      if (existe) {
        personaId = existe.id;
      } else {
        const persona = await prisma.persona.create({
          data: {
            primer_nombre: String(s.primer_nombre).toUpperCase(),
            segundo_nombre: s.segundo_nombre ? String(s.segundo_nombre).toUpperCase() : null,
            primer_apellido: String(s.primer_apellido).toUpperCase(),
            segundo_apellido: s.segundo_apellido ? String(s.segundo_apellido).toUpperCase() : null,
            nro_documento: String(s.nro_documento),
            tipo_documento: s.tipo_documento ? parseInt(s.tipo_documento) : 1,
            sexo: s.sexo ?? 'M',
            fecha_nacimiento: parseFecha(s.fecha_nacimiento) ?? new Date('1900-01-01')
          }
        });
        personaId = persona.id;
      }
      ...
      if (usuarioExiste) {
        ...
        const yaHab = await prisma.supervisor_habilitado.findFirst({ where: { usuario_id: usuarioId } });
        if (yaHab) {
          resultados.errores.push({ documento: s.nro_documento, motivo: 'Ya está habilitado como supervisor' });
          continue;
        }
      } else {
        const persona = await prisma.persona.findUnique({ where: { id: personaId } });
      ...
      await prisma.supervisor_habilitado.create({
        data: {
          ...
          nro_registro: s.nro_registro ? String(s.nro_registro) : null,
          fecha_vencimiento: parseFecha(s.fecha_vencimiento),
      ...
      console.error(err);
      resultados.errores.push({ documento: s.nro_documento, motivo: 'Error interno' });
    }
  }
...
const agregarContacto = async (req, res) => {
  ...
  try {
    const hab = await prisma.supervisor_habilitado.findUnique({
      where: { id: parseInt(id) },
      include: { usuario: { include: { persona: true } } }
    });
    if (!hab) return res.status(404).json({ error: 'Supervisor no encontrado' });
...
const editarContacto = async (req, res) => {
  ...
  const { tipo_contacto_id, valor, principal } = req.body;
  try {
    const hab = await prisma.supervisor_habilitado.findUnique({ where: { id: parseInt(id) }, include: { usuario: true } });
    if (!hab) return res.status(404).json({ error: 'Supervisor no encontrado' });
...
const actualizarRegistro = async (req, res) => {
  ...
  try {
    const hab = await prisma.supervisor_habilitado.findUnique({ where: { id: parseInt(id) } });
    if (!hab) return res.status(404).json({ error: 'Supervisor no encontrado' });
    const actualizado = await prisma.supervisor_habilitado.update({
      where: { id: parseInt(id) },
      data: {
...
const toggleActivo = async (req, res) => {
  ...
  try {
    const hab = await prisma.supervisor_habilitado.update({ where: { id: parseInt(id) }, data: { activo: Boolean(activo) } });
    res.json(hab);
  } catch (err) { console.error(err); res.status(500).json({ error: 'Error al actualizar supervisor' }); }
};
